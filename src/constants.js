...
export const furnitureLibrary = [
  ...
  { name: 'LED-Panel',      kategorie: 'Licht',      width: 40,  height: 40,  color: '#FAC775', border: '#BA7517' },
...
export function istDeckenleuchte(name) {
  ...
  return n.includes('deckenlampe') || n.includes('pendelleuchte') || n.includes('kronleuchter') || n.includes('spot') || n.includes('led-streifen') || n.includes('led-panel')
}

// Frei verschiebbare Deckenleuchten (Phase 6, Teilschritt 2+3+4): im Unterschied zu den fest/mittig
// platzierten Kronleuchter/Pendelleuchte/Deckenlampe oben lassen sich Spot, Spot-Reihe, LED-Streifen
// und LED-Panel frei auf der Decke verschieben (siehe RoomView3D.jsx deckenleuchteMausDown/-Move/-Up)
...
export function istVerschiebbareDeckenleuchte(name) {
  const n = name.toLowerCase()
  return n.includes('spot') || n.includes('led-streifen') || n.includes('led-panel')
}
...
// Deckenleuchten mit ziehbarem Eck-Anfasser (Phase 6, Teilschritt 4): bisher nur das quadratische
// LED-Panel — die Kantenlänge wird direkt in der 3D-Decken-Ansicht per Ziehen an einer Ecke
// eingestellt, symmetrisch um die (feststehende) Mitte. Anders als beim LED-Streifen oben ändern
// sich dabei Position UND Ausrichtung NICHT — nur die Größe.
export function istEckSkalierbareDeckenleuchte(name) {
  return name.toLowerCase().includes('led-panel')
}
...
const RAUM_OPTIONALE_FELDER = [
  ...
  'eckpunkte', 'furniture', 'trennwaende', 'boden', 'wandfarbe', 'wandfarben',
  'fussleiste', 'fussleisteFarbe', 'raumHoehe', 'tageszeit', 'wizardSchritt',
]
